...
async function generateMonth() {
  ...
  const ordered = nearestNeighborOrder(startCoords.lat, startCoords.lng, eligible);
  const chunks = [];
  for (let i = 0; i < ordered.length; i += stopCount) chunks.push(ordered.slice(i, i + stopCount));
  ...
  try {
    for (let i = 0; i < daysToUse; i++) {
      ...
      monthResults.push({ date: workingDays[i], stops: timing.stops, totalHours: timing.totalHours, usingRealRoads: timing.usingRealRoads });
    }
    ...
    setMonthStatus('Something went wrong generating the month. Try again.', 'error');
  } finally {
    genBtn.disabled = false;
...
function renderMonthResults() {
  ...
  container.innerHTML = monthResults.map((day, dayIdx) => {
...
function renderDayReviewSummary() {
  ...
  const totalMiles = computedStops.reduce((sum, s) => sum + (s.travelMiles || 0), 0);
  el.innerHTML = `
...
function renderDayReviewList() {
  ...
  el.innerHTML = dayReviewContext.computedStops.map((s, i) => `
...
function renderDayReviewMap() {
  ...
  const candidates = getFilteredPatients().filter(p => p.lat !== null && p.lng !== null && !scheduledIds.has(p.id));
  const ghostIcon = L.divIcon({ className: 'ghost-pin', html: '<div class="ghost-pin-dot"></div>', iconSize: [14, 14], iconAnchor: [7, 7] });
  ...
  candidates.forEach(p => {
    ...
    L.marker([p.lat, p.lng], { icon: ghostIcon }).addTo(dayReviewLayer).bindPopup(`
      ${escapeHtml(p.name)} (Group ${escapeHtml(p.group || '—')})<br>
...
function renderDayReviewSearchResults(term) {
  ...
  const matches = getFilteredPatients()
    .filter(p => p.lat !== null && p.lng !== null && !scheduledIds.has(p.id) && p.name.toLowerCase().includes(term.toLowerCase()))
    .slice(0, 8);
...
window.addToDayReview = async function (patientId) {
  ...
  const elsewhere = findPatientElsewhereInBatch(dayReviewContext.batchType, patientId, dayReviewContext.dayIdx);
  if (elsewhere) {
