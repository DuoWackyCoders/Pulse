...
function renderTable() {
  ...
  const displayed = getFilteredPatients().filter(
...
async function handleSaveNewAddress() {
  ...
  if (!address) {
    setScheduleStatus("Enter an address to save.", "error");
    return;
  }
  ...
  try {
    ...
    if (!coords) {
      setScheduleStatus("Could not find that address.", "error");
      return;
    }
    const saved = loadStartAddresses();
    const entry = {
...
function wireScheduleUI() {
  ...
  async function handleApproveClick() {
    ...
    const commit = () => {
      ...
      setScheduleStatus(
        `Schedule approved for ${scheduleDate} (${totalHours.toFixed(1)} hrs). Check the Home tab calendar to see it.`,
        "success",
      );
    };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("click", () => addFileInput.click());
  ...
  document
    .getElementById("clientsSearchInput")
