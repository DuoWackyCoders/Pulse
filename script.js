...
function populateProviderFilter() {
  ...
  const providers = Array.from(
    new Set(patients.map((p) => p.provider).filter(Boolean)),
  ).sort();
  sel.innerHTML =
    ...
    providers
      .map(
        (pr) =>
          `<option value="${escapeHtml(pr)}" ${pr === activeProviderFilter ? "selected" : ""}>${escapeHtml(pr)}</option>`,
      )
      .join("");
}
...
function csvToPatients(text) {
  ...
  const idx = {
    ...
    lastVisit: header.findIndex(
      (h) => h.includes("last visit") || h.includes("lastvisit"),
    ),
  };
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    out.push({
      ...
      coordinator:
        idx.coordinator >= 0 ? (cols[idx.coordinator] || "").trim() : "",
      provider: idx.provider >= 0 ? (cols[idx.provider] || "").trim() : "",
      lastVisitDate:
        idx.lastVisit >= 0
          ? normalizeDateString(cols[idx.lastVisit] || "")
          : "",
      extra,
      lat: null,
      lng: null,
...
function patientsToCSV(list) {
  ...
  for (const p of list) {
    const extraValues = extraColumns.map(
      (col) => (p.extra && p.extra[col]) || "",
    );
    lines.push(
      [
        p.name,
...
async function geocodeAllPending() {
  const pending = patients.filter(
    (p) => p.address && (p.lat === null || p.lng === null),
  );
  if (pending.length === 0) return;

  setStatus(
    `Geocoding ${pending.length} address(es)... this may take a moment.`,
    "",
  );
...
function regroup() {
  ...
  const auto = patients.filter(
    (p) => !p.manualGroup && p.lat !== null && p.lng !== null,
  );
  ...
  const nextFreeLetter = () => {
    let letter;
    do {
      letter = groupLetter(nextIdx++);
    } while (usedLabels.has(letter));
  ...
  for (let iter = 0; iter < 4; iter++) {
    ...
    auto.forEach((p) => {
      ...
      const currentDist = haversineMiles(
        centroids[p.group].lat,
        centroids[p.group].lng,
        p.lat,
        p.lng,
      );
      let bestGroup = p.group,
...
function populateHomeAddressSelect() {
  ...
  sel.innerHTML =
    '<option value="">No home set — using upload order</option>' +
    saved
      .map(
        (a) =>
          `<option value="${a.id}">${escapeHtml(a.label)} — ${escapeHtml(a.address)}</option>`,
      )
      .join("");
...
function ensureClientsMap() {
  ...
  clientsMap.on(L.Draw.Event.CREATED, (e) => {
    ...
    pendingDrawSelection = getFilteredPatients().filter(
      (p) =>
        p.lat !== null &&
        p.lng !== null &&
    ...
    document.getElementById("drawAssignCount").textContent =
      `${pendingDrawSelection.length} patient(s) selected —`;
...
function renderClientsMap() {
  ...
  const withCoords = getFilteredPatients().filter(
  ...
  withCoords.forEach((p) => {
    const groupOptions = availableGroupLetters()
      .map(
        (l) =>
          `<option value="${l}" ${p.group === l ? "selected" : ""}>Group ${l}</option>`,
      )
      .join("");
    const popupHtml = `
      <div style="min-width:170px;">
        ...
        <button type="button" style="margin-top:8px; font-size:0.8rem; padding:4px 8px; width:100%;" onclick="window.saveClientGroupFromMap('${p.id}')">Save</button>
      </div>
    `;
...
window.saveClientGroupFromMap = function (patientId) {
  ...
  if (!label) {
    alert("Type a group label first.");
    return;
  }
...
function wireClientsMap() {
  document.getElementById("drawAssignBtn").addEventListener("click", () => {
    ...
    if (!label) {
      alert("Type a group label first (e.g. A).");
      return;
    }
    if (pendingDrawSelection.length === 0) {
      alert("No patients were inside that circle.");
      return;
    }
...
function renderTable() {
  ...
  const displayed = providerScoped.filter(
    (p) => !searchTerm || p.name.toLowerCase().includes(searchTerm),
  );
  ...
  if (countEl) {
    ...
    if (isFiltered) {
      ...
      if (activeProviderFilter)
        reasons.push(`Viewing as: ${activeProviderFilter}`);
  ...
  emptyState.innerHTML =
    "<p>No patients yet.</p><p>Upload a CSV above to get started.</p>";
  ...
  displayed.forEach((p) => {
    ...
    const extraCells = extraColumns
      .map((col) => `<td>${escapeHtml(p.extra && p.extra[col])}</td>`)
      .join("");
    ...
    tr.innerHTML = `
      ...
      <td>
        <select class="group-select" data-id="${p.id}">
          ...
          ${availableGroupLetters()
            .map(
              (l) =>
                `<option value="${l}" ${p.manualGroup && p.group === l ? "selected" : ""}>${l}</option>`,
            )
            .join("")}
        </select>
      </td>
...
function downloadAllPatientsCsv() {
  ...
  a.href = url;
  a.download = activeProviderFilter
    ? `patients-${activeProviderFilter}.csv`
    : "patients.csv";
  a.click();
...
function showClearAllStep1() {
  ...
  document.getElementById("clearAllTitle").textContent =
  ...
  document.getElementById("clearAllBackOut").addEventListener("click", () => {
    modal.style.display = "none";
  });
  document
    .getElementById("clearAllYes")
    .addEventListener("click", showClearAllStep2);
}
...
function addManualRow() {
  manualRowCount++;
  document
    .getElementById("manualAddRows")
    .insertAdjacentHTML("beforeend", manualRowHtml(manualRowCount));
}
...
function checkHiddenByCurrentView(addedList) {
  const reasons = [];
  if (activeProviderFilter) {
    const hiddenByProvider = addedList.filter(
      (p) => p.provider !== activeProviderFilter,
    );
    if (hiddenByProvider.length > 0) {
      reasons.push(
        `${hiddenByProvider.length} won't show under "Viewing as: ${activeProviderFilter}" (their Provider field doesn't match) — switch to "All Providers" to see them`,
      );
    }
  }
  const searchInput = document.getElementById("clientsSearchInput");
  const searchTerm = searchInput ? searchInput.value.trim().toLowerCase() : "";
  if (searchTerm) {
    const hiddenBySearch = addedList.filter(
      (p) => !p.name.toLowerCase().includes(searchTerm),
    );
    if (hiddenBySearch.length > 0) {
      reasons.push(
        `${hiddenBySearch.length} won't show under the current Clients search — clear the search box to see them`,
      );
    }
  }
  return reasons;
...
async function submitManualAdd() {
  ...
  rows.forEach((row) => {
    const get = (f) =>
      row.querySelector(`input[data-field="${f}"]`).value.trim();
    const name = get("name");
  ...
  if (candidates.length === 0) {
    document.getElementById("manualAddStatus").textContent =
      "Fill in at least a name and address on one row.";
  ...
  candidates.forEach((c) => {
    if (isDuplicatePatient(c, patients) || isDuplicatePatient(c, toAdd))
      skipped++;
    else toAdd.push(c);
  });
  ...
  if (!saveOk) {
    statusEl.textContent =
      "Save failed — your browser storage may be full. The patient(s) may not persist after reload.";
    statusEl.className = "status-line error";
  } else {
    const hiddenReasons = checkHiddenByCurrentView(toAdd);
    const hiddenNote =
      hiddenReasons.length > 0 ? " ⚠️ " + hiddenReasons.join("; ") + "." : "";
    statusEl.textContent =
      `Added ${toAdd.length} patient(s).` +
...
function handleFile(file, mode) {
  ...
  reader.onload = async (e) => {
    ...
    if (parsed.length === 0) {
      setStatus(
        "No valid rows found in that CSV. Check the column headers (Name, Address, DOB...).",
        "error",
      );
      return;
    }
    ...
    if (mode === "replace") {
      ...
      parsed.forEach((candidate) => {
        if (
          isDuplicatePatient(candidate, patients) ||
          isDuplicatePatient(candidate, toAdd)
        ) {
          skippedDupes++;
        } else {
    ...
    if (!saveOk) {
      setStatus(
        "Save failed — your browser storage may be full. This data may not persist after reload.",
        "error",
      );
    } else {
      ...
      const hiddenNote =
        hiddenReasons.length > 0 ? " ⚠️ " + hiddenReasons.join("; ") + "." : "";
      setStatus(
        `Loaded ${addedCount} patient(s).` +
          (skippedDupes > 0
            ? ` Skipped ${skippedDupes} duplicate(s) (matching name, address, and DOB).`
            : "") +
          hiddenNote +
          " Geocoding addresses next...",
        hiddenNote ? "error" : "success",
      );
    }
    await geocodeAllPending();
  };
...
function renderMap() {
  ...
  scheduledPatients.forEach((p, i) => {
    L.marker([p.lat, p.lng]).addTo(leafletLayer).bindPopup(`
  ...
  if (lastRouteGeometry && lastRouteGeometry.length > 1) {
    ...
    // not an actual path.
    L.polyline(points, { color: "#EE7EAB", weight: 3, dashArray: "1,8" }).addTo(
      leafletLayer,
    );
  }
  ...
  const scheduleDateForMap =
    document.getElementById("scheduleDate").value ||
    new Date().toISOString().slice(0, 10);
  const includeRecentForMap = document.getElementById("includeRecent").checked;
  const otherPatients = getFilteredPatients().filter(
    (p) => p.lat !== null && p.lng !== null && !scheduledIds.has(p.id),
  );
  ...
  otherPatients.forEach((p) => {
    const ineligible =
      !includeRecentForMap && isRecentlyVisited(p, scheduleDateForMap);
...
async function reorderAndRecalc() {
  if (startCoords && scheduledPatients.length > 1) {
    scheduledPatients = nearestNeighborOrder(
      startCoords.lat,
      startCoords.lng,
      scheduledPatients,
    );
...
window.addPatientToScheduleDirectly = async function (patientId) {
  if (!startCoords) {
    alert(
      "Generate a route first (need a starting address set) before adding a patient directly.",
    );
    return;
  }
  ...
  if (!includeRecent && isRecentlyVisited(p, scheduleDate)) {
    alert(
      `${p.name} was visited within the last 30 days. Check "Include patients visited in the last 30 days" to add them anyway.`,
    );
    return;
  }
  ...
  setScheduleStatus(
    `Added ${p.name} to the route — order re-optimized, times recalculated.`,
    "success",
  );
};
...
window.removeFromRouteToLeftover = async function (patientId) {
  ...
  setScheduleStatus(
    `Removed ${removed.name} from the route — moved to Leftover, remaining times recalculated.`,
    "",
  );
};
...
async function addStartAddress(label, address, statusEl) {
  if (!address) {
    if (statusEl) {
      statusEl.textContent = "Enter an address to save.";
      statusEl.className = "status-line error";
    }
    return null;
  }
  ...
  try {
    ...
    if (!coords) {
      if (statusEl) {
        statusEl.textContent = "Could not find that address.";
        statusEl.className = "status-line error";
      }
      return null;
    }
    ...
    if (statusEl) {
      statusEl.textContent = "Address saved.";
      statusEl.className = "status-line success";
    }
    return entry;
  } catch (e) {
    if (statusEl) {
      statusEl.textContent = "Error looking up that address.";
      statusEl.className = "status-line error";
    }
    return null;
  }
...
async function handleSaveNewAddress() {
  const label =
    document.getElementById("newAddressLabel").value.trim() || "Home base";
...
function groupSelectOptionsHtml() {
  ...
  return (
    '<option value="__ANY__">🔀 Closest Mix (any group)</option>' +
...
function populateGroupSelect() {
  ...
  if (current && (current === "__ANY__" || groups.includes(current)))
    sel.value = current;
...
function isSameLocation(a, b) {
  ...
  if (a.address.trim().toLowerCase() === b.address.trim().toLowerCase())
    return true;
...
function selectRoutePatients({
  ...
  const allEligible = getFilteredPatients().filter(
    (p) => p.lat !== null && p.lng !== null && !excludeSet.has(p.id),
  );
  const eligible = includeRecent
    ? allEligible
    : allEligible.filter((p) => !isRecentlyVisited(p, scheduleDate));
  ...
  if (strictGroup) {
    const groupPatients = eligible.filter((p) =>
      targetGroups.includes(p.group),
    );
    ...
    if (groupPatients.length < stopCount) {
      ...
      const otherByDistance = eligible
        .filter((p) => !targetGroups.includes(p.group))
        .map((p) => ({
          p,
          dist: haversineMiles(startCoords.lat, startCoords.lng, p.lat, p.lng),
        }))
  ...
  const addressMates = byDistance
    ...
    .filter(
      (p) =>
        !closestN.includes(p) && closestN.some((sel) => isSameLocation(sel, p)),
    );
  closestN = closestN.concat(addressMates);

  const remainder = byDistance
    .map((x) => x.p)
    .filter((p) => !closestN.includes(p));
 ...
 */
async function computeTimingForDay(
  startCoords,
  orderedPatients,
  startTimeStr,
  ...
  const legsResult = await fetchRouteLegs(
    startCoords.lat,
    startCoords.lng,
    orderedPatients,
  );
...
function renderWeekDayCards() {
  ...
  container.querySelectorAll(".wdc-work-anyway").forEach((cb) => {
    cb.addEventListener("change", (e) => {
      ...
      card.querySelector(".wdc-fields").style.display = e.target.checked
        ? "block"
        : "none";
    });
  });
...
async function generateWeek() {
  ...
  if (!startAddrId) {
    setWeekStatus(
      "Pick a starting address first (add one on the Daily tab if needed).",
      "error",
    );
    return;
  }
  ...
  if (!saved) {
    setWeekStatus("That saved address could not be found.", "error");
    return;
  }
  ...
  setWeekStatus(
    "Working through Monday–Friday, one real-road route at a time...",
    "",
  );
  ...
  try {
    ...
    for (let i = 0; i < dayCards.length; i++) {
      ...
      const workingToday =
        !isStandardOff || (workAnywayCb && workAnywayCb.checked);
      ...
      const stopCount =
        parseInt(card.querySelector(".wdc-count").value, 10) || 0;
      ...
      const timing = await computeTimingForDay(
        weekStartCoords,
        selection.scheduled,
        startTime,
        visitDuration,
    ...
    setWeekStatus(
      `Generated ${successDays} of 5 days. Review below, then Approve Whole Week.`,
      "success",
    );
    document.getElementById("approveWeekBtn").style.display = weekResults.some(
      (d) => !d.error,
    )
      ? "inline-block"
      : "none";
    ...
    setWeekStatus(
      "Something went wrong generating the week. Try again.",
      "error",
    );
  } finally {
...
window.openWeekDayInGoogleMaps = function (dayIdx) {
  ...
  if (day.stops.length > 23) {
    alert(
      "Google Maps supports up to ~23 stops in one link — this day has more. Split it or use My Maps export instead.",
    );
    return;
  }
...
function renderWeekResults() {
  ...
  container.innerHTML = weekResults
    .map((day, dayIdx) => {
      if (day.offDay) {
        return `
      ...
      const fillNote =
        day.fillCount > 0
          ? ` — ${day.fillCount} pulled from nearby groups to fill the count`
          : "";
      const excludedNote =
        day.excludedCount > 0
          ? ` — ${day.excludedCount} excluded (visited &lt;30 days)`
          : "";
      return `
...
function approveWeek() {
  ...
  const allPatientsThisWeek = weekResults
    ...
    .flatMap((d) => d.stops);
  const doubleBooked = findDoubleBookedPatients(
    allPatientsThisWeek,
    datesToApprove,
  );

  if (doubleBooked.length > 0) {
    const list = doubleBooked
      .map((c) => `${c.name} (already on ${c.date})`)
      .join(", ");
    showOverwriteConfirm(
...
function commitApproveWeek() {
  ...
  renderTable();
  setWeekStatus(
    `Approved the week — ${totalApproved} patient visit(s) across the days that generated successfully. Check the Home tab calendar.`,
    "success",
  );
...
function wireAdminTab() {
  ...
  document
    .getElementById("adminAddAddressBtn")
    .addEventListener("click", async () => {
      const label = document
        .getElementById("adminNewAddressLabel")
        .value.trim();
      const address = document
        .getElementById("adminNewAddressText")
        .value.trim();
  ...
  document
    .getElementById("recalcLastVisitBtn")
...
function switchScheduleMode(mode) {
  document.getElementById("scheduleModeDaily").style.display =
    mode === "daily" ? "block" : "none";
  document.getElementById("scheduleModeWeekly").style.display =
    mode === "weekly" ? "block" : "none";
  document.getElementById("scheduleModeMonthly").style.display =
    mode === "monthly" ? "block" : "none";
  document
    .getElementById("dailyModeBtn")
    .classList.toggle("active", mode === "daily");
  document
    .getElementById("weeklyModeBtn")
  ...
  if (mode === "monthly") {
    ...
    document.getElementById("monthGroupSelect").innerHTML =
...
function wireWeeklyUI() {
  document
    .getElementById("dailyModeBtn")
    .addEventListener("click", () => switchScheduleMode("daily"));
  document
    .getElementById("weeklyModeBtn")
    .addEventListener("click", () => switchScheduleMode("weekly"));
  document
    .getElementById("weekStartDate")
    .addEventListener("change", renderWeekDayCards);
  document
    .getElementById("generateWeekBtn")
    .addEventListener("click", generateWeek);
  document
    .getElementById("approveWeekBtn")
    .addEventListener("click", approveWeek);
  document
    .getElementById("cancelWeekBtn")
    .addEventListener("click", cancelWeek);
...
/* ============================================
   ...
   ============================================ */
let monthResults = []; // [{date, stops, totalHours, usingRealRoads, emptyGroup?}]
...
function renderMonthWeekPatternRows() {
  ...
  weeks.forEach((week, idx) => {
    monthWeekPatterns[idx] = [...standardWorkDays];
  }); // pre-fill from the usual pattern, adjustable per week below
...
async function generateMonth() {
  ...
  const saved = loadStartAddresses().find((a) => a.id === startAddrId);
  if (!saved) {
    setMonthStatus("Pick a starting address first.", "error");
    return;
  }
  ...
  const stopCount =
    parseInt(document.getElementById("monthStopCount").value, 10) || 8;
  ...
  if (workingDays.length === 0) {
    setMonthStatus(
      "No working days in that month based on your Standard Work Days settings.",
      "error",
    );
    return;
  }
  ...
  const groupPatients = isMix
    ? allEligible
    : allEligible.filter((p) => p.group === group);
  ...
  const eligible = includeRecent
    ? groupPatients
    : groupPatients.filter((p) => !isRecentlyVisited(p, workingDays[0]));

  if (eligible.length === 0) {
    setMonthStatus(
      "No eligible patients found for that group this month.",
      "error",
    );
    return;
  }
  ...
  genBtn.disabled = true;
  genBtn.textContent =
    "⏳ Generating month (this takes a while — one real-road fetch per working day)...";
  setMonthStatus(
    `Ordering ${eligible.length} patient(s) geographically...`,
    "",
  );
  ...
  const chunks = [];
  for (let i = 0; i < ordered.length; i += stopCount) {
    const chunk = ordered.slice(i, i + stopCount);
    chunks.push(
  ...
  try {
    ...
    renderMonthResults();
    const scheduledCount = monthResults.reduce(
      (sum, d) => sum + ((d.stops && d.stops.length) || 0),
      0,
    );
    const leftoverCount = ordered.length - scheduledCount;
    let msg = `Generated ${daysToUse} working day(s) covering ${scheduledCount} of ${ordered.length} eligible patient(s).`;
    if (leftoverCount > 0)
      msg += ` ${leftoverCount} patient(s) didn't fit this month — still due, pick them up next month or on an ad-hoc day.`;
    ...
    setMonthStatus(
      "Something went wrong generating the month. Try again.",
      "error",
    );
  } finally {
    genBtn.disabled = false;
...
function renderMonthResults() {
  ...
  container.innerHTML = monthResults
    .map((day, dayIdx) => {
      const d = new Date(day.date + "T00:00:00");
      const label = `${dayNames[d.getDay()]} ${day.date}`;
      if (day.emptyGroup) {
      ...
      <div class="week-result-card">
        ...
        <p class="wrc-meta">${day.stops.length} stop(s) · ${day.totalHours.toFixed(1)} hrs · ${day.usingRealRoads ? "✓ real road times" : "⚠ straight-line estimate"}</p>
        ${day.stops
          .map(
            (s, i) => `
          <div class="week-stop-row">
...
window.openMonthDayInGoogleMaps = function (dayIdx) {
  ...
  if (!day || !day.stops || day.stops.length === 0 || !monthStartCoordsGlobal) {
    alert("No stops to map for this day.");
    return;
  }
  if (day.stops.length > 23) {
    alert(
      "Google Maps supports up to ~23 stops in one link — this day has more.",
    );
    return;
  }
...
function approveMonth() {
  const datesToApprove = monthResults
    .filter((d) => !d.emptyGroup && d.stops && d.stops.length > 0)
    .map((d) => d.date);
  const conflicts = datesWithExistingSchedule(datesToApprove);
  ...
  const allPatientsThisMonth = monthResults
    .filter((d) => !d.emptyGroup && d.stops)
    .flatMap((d) => d.stops);
  const doubleBooked = findDoubleBookedPatients(
    allPatientsThisMonth,
    datesToApprove,
  );
  if (doubleBooked.length > 0) {
    const list = doubleBooked
...
function commitApproveMonth() {
  ...
  renderTable();
  setMonthStatus(
    `Approved the month — ${totalApproved} patient visit(s) scheduled. Check the Home tab calendar.`,
    "success",
  );
...
function findPatientElsewhereInBatch(batchType, patientId, excludeDayIdx) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const day = results[i];
    if (day.stops && day.stops.some((s) => s.id === patientId))
      return { dayIdx: i, date: day.date };
  }
  return null;
}
...
window.openDayReviewModal = function (batchType, dayIdx) {
  ...
  const startCoords =
    batchType === "week" ? weekStartCoordsGlobal : monthStartCoordsGlobal;
  const startTime =
    batchType === "week"
  ...
  if (!startCoords) {
    alert("Missing starting address for this batch — regenerate first.");
    return;
  }
  ...
  document.getElementById("dayReviewTitle").textContent =
    `${getBatchDayLabel(day.date)} — Review & Adjust`;
...
async function recalcDayReview() {
  ...
  document.getElementById("dayReviewSummary").innerHTML =
    '<span class="rs-item">⏳ Recalculating...</span>';
...
function renderDayReviewMap() {
  ...
  L.marker([startCoords.lat, startCoords.lng], { icon: homeIcon })
    .addTo(dayReviewLayer)
    .bindPopup("Start (home base)");

  computedStops.forEach((s, i) => {
    L.marker([s.lat, s.lng]).addTo(dayReviewLayer).bindPopup(`
  ...
  const candidates = getFilteredPatients().filter(
    (p) => p.lat !== null && p.lng !== null && !scheduledIds.has(p.id),
  );
  ...
  candidates.forEach((p) => {
    const elsewhere = findPatientElsewhereInBatch(batchType, p.id, dayIdx);
    const label = elsewhere
      ? `currently on ${getBatchDayLabel(elsewhere.date)}`
      : "unassigned";
    L.marker([p.lat, p.lng], { icon: ghostIcon }).addTo(dayReviewLayer)
      .bindPopup(`
      ${escapeHtml(p.name)} (Group ${escapeHtml(p.group || "—")})<br>
      <span style="color:var(--text-soft); font-size:0.8rem;">${label}</span>
...
function renderDayReviewSearchResults(term) {
  ...
  container.innerHTML = matches
    .map((p) => {
      const elsewhere = findPatientElsewhereInBatch(
        dayReviewContext.batchType,
        p.id,
        dayReviewContext.dayIdx,
      );
      const label = elsewhere
        ? `currently on ${getBatchDayLabel(elsewhere.date)}`
        : "unassigned";
...
window.addToDayReview = async function (patientId) {
  ...
  if (elsewhere) {
    dayReviewContext.pendingPulls[patientId] = elsewhere.dayIdx;
  }

  dayReviewContext.scheduled.push(p);
  // Re-optimize order — same "auto restructure" logic used everywhere else a stop is added mid-route.
  dayReviewContext.scheduled = nearestNeighborOrder(
    dayReviewContext.startCoords.lat,
...
window.removeFromDayReview = async function (patientId) {
  if (!dayReviewContext) return;
  dayReviewContext.scheduled = dayReviewContext.scheduled.filter(
    (s) => s.id !== patientId,
  );
...
window.confirmDayReview = async function () {
  ...
  try {
    ...
    for (const originDayIdx of affectedOriginDays) {
      ...
      const remaining = originDay.stops.filter(
        (s) => pendingPulls[s.id] !== originDayIdx,
      );
      const originStartCoords =
        batchType === "week" ? weekStartCoordsGlobal : monthStartCoordsGlobal;
      const originStartTime =
        batchType === "week"
          ? document.getElementById("weekStartTime").value || "08:00"
          : document.getElementById("monthStartTime").value || "08:00";
      const originVisitDuration =
        batchType === "week"
          ? parseFloat(document.getElementById("weekVisitDuration").value) || 15
          : parseFloat(document.getElementById("monthVisitDuration").value) ||
            15;
    ...
    if (batchType === "week") renderWeekResults();
    else renderMonthResults();
...
function wireDayReviewUI() {
  document
    .getElementById("dayReviewCloseX")
    .addEventListener("click", window.cancelDayReview);
  document
    .getElementById("dayReviewCancelBtn")
    .addEventListener("click", window.cancelDayReview);
  document
    .getElementById("dayReviewConfirmBtn")
    .addEventListener("click", window.confirmDayReview);
  document
    .getElementById("dayReviewSearchInput")
    .addEventListener("input", (e) =>
      renderDayReviewSearchResults(e.target.value.trim()),
    );
}
...
function wireMonthlyUI() {
  document
    .getElementById("monthlyModeBtn")
    .addEventListener("click", () => switchScheduleMode("monthly"));
  document
    .getElementById("generateMonthBtn")
    .addEventListener("click", generateMonth);
  document
    .getElementById("approveMonthBtn")
    .addEventListener("click", approveMonth);
  document
    .getElementById("cancelMonthBtn")
    .addEventListener("click", cancelMonth);
  document
...
async function generateRoute() {
  ...
  const group2 =
    group !== "__ANY__" && group2Raw && group2Raw !== group ? group2Raw : null;
  const stopCount =
    parseInt(document.getElementById("stopCount").value, 10) || 0;
  ...
  if (!group) {
    setScheduleStatus(
      "Pick a group first — none found. Upload patients and set a radius on the Clients tab.",
      "error",
    );
    return;
  }
  if (!startAddrId) {
    setScheduleStatus("Pick or add a starting address.", "error");
    return;
  }
  ...
  setScheduleStatus(
    "Looking up real road routes (up to 8 seconds, then falls back automatically if unavailable)...",
    "",
  );
  ...
  try {
    ...
    if (!saved) {
      setScheduleStatus(
        "That saved address could not be found — try re-adding it.",
        "error",
      );
      return;
    }
    ...
    const result = selectRoutePatients({
      group,
      group2,
      stopCount,
      startCoords,
    ...
    if (result.error) {
      setScheduleStatus(result.error, "error");
      return;
    }
    ...
    await recalcAndRender();
    const addedNote =
      result.addressMateCount > 0
        ? ` (+${result.addressMateCount} same-address patient(s) added automatically.)`
        : "";
    const excludedNote =
      result.excludedCount > 0
        ? ` (${result.excludedCount} recently-visited patient(s) excluded.)`
        : "";
    let statusMsg;
    if (result.strictGroup) {
      const fillNote =
        result.fillCount > 0
          ? ` Group ${group} only had ${result.groupOnlyCount} available, so ${result.fillCount} nearby patient(s) from other groups were added to reach ${stopCount}.`
          : "";
      statusMsg =
        `Route generated for Group ${group}${group2 ? " + " + group2 : ""}.` +
        excludedNote +
        addedNote +
        fillNote;
    } else {
      const groupsUsed = Array.from(
        new Set(scheduledPatients.map((p) => p.group || "unassigned")),
      ).sort();
      statusMsg =
        `Route generated: closest ${scheduledPatients.length} patient(s) to your starting address, drawn from group(s) ${groupsUsed.join(", ")}.` +
        excludedNote +
        addedNote;
    }
    ...
    setScheduleStatus(
      "Something went wrong generating the route. Try again.",
      "error",
    );
  } finally {
    genBtn.disabled = false;
...
async function fetchRouteLegs(startLat, startLng, stops) {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    console.error(
      "Real-road routing unavailable (timed out or failed), falling back to straight-line estimate",
      e,
    );
    return null;
  }
...
async function recalcAndRender() {
  ...
  const visitDuration =
    parseFloat(document.getElementById("visitDuration").value) || 15;
  ...
  if (startCoords && scheduledPatients.length > 0) {
    const result = await fetchRouteLegs(
      startCoords.lat,
      startCoords.lng,
      scheduledPatients,
    );
...
function renderScheduleLists() {
  ...
  document.getElementById("leftoverCount").textContent =
    leftoverPatients.length;

  schedEl.innerHTML = scheduledPatients
    .map((p, i) => dragItemHtml(p, "scheduled", i, true))
    .join("");
  leftEl.innerHTML = leftoverPatients
    .map((p, i) => dragItemHtml(p, "leftover", i, false))
    .join("");
...
function handleDrop(id, fromList, toList, event, listEl) {
  const fromArr =
    fromList === "scheduled" ? scheduledPatients : leftoverPatients;
  ...
  scheduledPatients.forEach((p, i) => {
    lines.push(
      [
        ...
        p.name,
        p.address,
        minutesToClock(p.arrivalMinutes),
        p.group || "",
      ]
        .map(escape)
        .join(","),
...
function openInGoogleMaps() {
  ...
  if (scheduledPatients.length > 23) {
    setScheduleStatus(
      "Google Maps supports up to ~23 stops in one link — trim the route or split it into two trips.",
      "error",
    );
    return;
  }
...
function cancelRoute() {
  resetRouteBuilderUI();
  setScheduleStatus(
    "Route cleared. Adjust your settings and generate again.",
    "",
  );
}
...
function wireScheduleUI() {
  ...
  document
    ...
    .addEventListener("click", cancelRoute);
  document
    .getElementById("openGoogleMapsBtn")
    .addEventListener("click", openInGoogleMaps);
  document
    .getElementById("myMapsCsvBtn")
    .addEventListener("click", downloadMyMapsCsv);

  document
    .getElementById("startAddressSelect")
    .addEventListener("change", updateAddressFormVisibility);
  document
    .getElementById("groupSelect")
    .addEventListener("change", updateGroup2Availability);
  document
    .getElementById("saveNewAddressBtn")
    .addEventListener("click", handleSaveNewAddress);
  ...
  document
    .getElementById("cancelNewAddressBtn")
    .addEventListener("click", () => {
      document.getElementById("newAddressLabel").value = "";
      document.getElementById("newAddressText").value = "";
      const saved = loadStartAddresses();
  ...
  document
    .getElementById("exportScheduleCsvBtn")
    .addEventListener("click", () => {
      if (scheduledPatients.length === 0) {
        setScheduleStatus("Generate a route first.", "error");
        return;
  ...
  async function handleApproveClick() {
    ...
    const commit = () => {
      ...
      renderTable();
      setScheduleStatus(
        `✅ Schedule approved for ${scheduleDate} (${totalHours.toFixed(1)} hrs). Check the Home tab calendar to see it. Ready for the next route.`,
        "success",
      );
      resetRouteBuilderUI();
    };
    ...
    const doubleBooked = findDoubleBookedPatients(scheduledPatients, [
      scheduleDate,
    ]);
    if (doubleBooked.length > 0) {
      const list = doubleBooked
  ...
  document
    .getElementById("approveScheduleBtn")
    .addEventListener("click", handleApproveClick);
  document
    .getElementById("approveScheduleBtnTop")
    .addEventListener("click", handleApproveClick);
}
...
async function saveEditedPatient() {
  ...
  if (addressChanged) {
    ...
    try {
      ...
      if (coords) {
        p.lat = coords.lat;
        p.lng = coords.lng;
        p.geocodeFailed = false;
      } else {
        p.lat = null;
        p.lng = null;
        p.geocodeFailed = true;
        statusEl.textContent =
          "Address still not found — saved anyway, but it won't be groupable until fixed.";
      }
    } catch (e) {
      p.geocodeFailed = true;
    }
  ...
  renderTable();
  populateProviderFilter();
  if (addressChanged) regroup();
  else renderGroupSummary();
...
function wireEditModal() {
  document
    ...
    .addEventListener("click", saveEditedPatient);
...
function recomputeAllLastVisitDates() {
  ...
  Object.keys(schedules).forEach((dateStr) => {
    schedules[dateStr].forEach((entry) => {
      ...
      if (!key) return;
      if (!mostRecentByName[key] || dateStr > mostRecentByName[key])
        mostRecentByName[key] = dateStr;
    });
  });
  let changed = 0;
  patients.forEach((p) => {
...
function showOverwriteConfirm(message, onConfirm) {
  ...
  modal.style.display = "flex";
  document
    .getElementById("overwriteCancelBtn")
    .addEventListener("click", () => {
      modal.style.display = "none";
    });
  document
    .getElementById("overwriteConfirmBtn")
    .addEventListener("click", () => {
      modal.style.display = "none";
      onConfirm();
    });
...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (count > 0) {
      const groupsUsed = Array.from(
        new Set(dayList.map((p) => p.group || "unassigned")),
      );
      ...
      if (groupsUsed.length > 1) {
        dotsHtml = `<div class="cal-day-dots">${groupsUsed
          .slice(0, 5)
          .map(
            (g) =>
              `<span class="cal-dot" style="background:${groupColor(g)}"></span>`,
          )
          .join("")}</div>`;
      }
    }
  ...
  dayCells.forEach((cell) => {
    cell.addEventListener("click", () =>
      showCalendarDay(cell.getAttribute("data-date")),
    );
...
function moveWholeDay(fromDate, toDate) {
  ...
  if (schedules[toDate] && schedules[toDate].length > 0) {
    if (
      !confirm(
        `${toDate} already has a schedule. Overwrite it with ${fromDate}'s ${fromList.length} patient(s)?`,
      )
    )
      return;
  }
...
window.cancelDaySchedule = function (dateStr) {
  ...
  if (list.length === 0) {
    alert("Nothing scheduled on this day.");
    return;
  }
  if (
    !confirm(
      `Cancel the entire schedule for ${dateStr}? This removes all ${list.length} patient(s) from the calendar for this day and cannot be undone.`,
    )
  )
    return;
...
window.cancelWeekSchedule = function (dateStr) {
  ...
  const schedules = loadSchedules();
  const totalPatients = weekDates.reduce(
    (sum, d) => sum + (schedules[d] ? schedules[d].length : 0),
    0,
  );
  if (totalPatients === 0) {
    alert("Nothing scheduled that week.");
    return;
  }
  if (
    !confirm(
      `Cancel the entire week's schedule (${weekDates[0]} through ${weekDates[4]})? This removes all ${totalPatients} patient visit(s) across those 5 days and cannot be undone.`,
    )
  )
    return;
  ...
  const resolved = list
    .map((entry) =>
      entry.lat != null && entry.lng != null
        ? entry
        : { ...entry, ...(patients.find((p) => p.id === entry.id) || {}) },
    )
    .filter((e) => e.lat != null && e.lng != null);

  if (resolved.length === 0) {
    alert(
      "Could not find coordinates for these patients — they may have been edited or removed since.",
    );
    return;
  }
  if (resolved.length > 23) {
    alert(
      "Google Maps supports up to ~23 stops in one link — this day has more.",
    );
    return;
...
window.openEditArrivalLive = function (patientId) {
  ...
  document.getElementById("editArrivalSubtitle").textContent =
    `${p.name} — scheduled arrival, adjust for known delays (traffic, a longer prior visit, etc.)`;
  document.getElementById("editArrivalInput").value = minutesToTimeInputValue(
    p.arrivalMinutes,
  );
  document.getElementById("editArrivalNote").textContent =
    "Everyone after this stop will shift to start from the new time.";
  document.getElementById("editArrivalNote").className = "status-line";
  document.getElementById("editArrivalResetBtn").style.display =
    p.manualArrivalOverride != null ? "inline-block" : "none";
...
window.openEditArrivalCalendar = function (dateStr, patientId) {
  ...
  document.getElementById("editArrivalSubtitle").textContent =
    `${entry.name} — record the actual arrival time for this visit.`;
  document.getElementById("editArrivalInput").value = minutesToTimeInputValue(
    entry.arrivalMinutes,
  );
  document.getElementById("editArrivalNote").textContent =
    "Every patient scheduled after this one that day will shift by the same amount (e.g. this visit ran long, so the rest of the day moves too).";
  document.getElementById("editArrivalNote").className = "status-line";
...
async function commitArrivalEdit() {
  ...
  const newMinutes = timeInputValueToMinutes(
    document.getElementById("editArrivalInput").value,
  );

  if (pendingArrivalEdit.context === "live") {
    const p = scheduledPatients.find(
      (sp) => sp.id === pendingArrivalEdit.patientId,
    );
    if (p) {
    ...
    if (idx !== -1) {
      ...
      for (let i = idx; i < list.length; i++) {
        if (list[i].arrivalMinutes !== undefined)
          list[i].arrivalMinutes += delta;
      }
      saveSchedules(schedules);
    }
    document.getElementById("editArrivalModal").style.display = "none";
...
window.changePatientDate = function (dateStr, patientId) {
  ...
  pendingDateChange = {
    fromDate: dateStr,
    patientId,
    name: master ? master.name : "this patient",
  };

  document.getElementById("changeDateSubtitle").textContent =
    `Moving ${pendingDateChange.name} — currently on ${dateStr}`;
...
function downloadScheduleCsv(entries, filename) {
  if (entries.length === 0) {
    alert("Nothing to export for that range.");
    return;
  }
  const header = ["Name", "DOB", "Date", "Time", "Provider"];
  const escape = (v) => {
  ...
  dates.forEach((date, dIdx) => {
    ...
    const baseMinutes =
      dayEntries[0].arrivalMinutes !== undefined
    ...
    dayEntries.forEach((e, i) => {
      ...
      lines.push(
        [e.name, e.dob, e.date, minutesToClock(slotMinutes), e.provider || ""]
          .map(escape)
          .join(","),
      );
    });
...
function exportMonthSchedule() {
  ...
  downloadScheduleCsv(
    entries,
    `schedule-${y}-${String(m + 1).padStart(2, "0")}.csv`,
  );
}
...
function exportWeekSchedule(anchorDateStr) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (schedules[key])
      schedules[key].forEach((p) => entries.push({ ...p, date: key }));
  }
...
function exportDaySchedule(dateStr) {
  const schedules = loadSchedules();
  const entries = (schedules[dateStr] || []).map((p) => ({
    ...p,
    date: dateStr,
  }));
...
function wireCalendarUI() {
  ...
  document
    ...
    .addEventListener("click", exportMonthSchedule);
  ...
  document
    .getElementById("dayDetailGoogleMapsBtn")
    .addEventListener("click", () => {
      const modal = document.getElementById("dayDetailModal");
      const dateStr = modal.getAttribute("data-current-date");
  ...
  document
    .getElementById("changeDateConfirmBtn")
    .addEventListener("click", commitPatientDateChange);
  document
    .getElementById("changeDateCancelBtn")
    .addEventListener("click", () => {
      document.getElementById("changeDateModal").style.display = "none";
      pendingDateChange = null;
    });
  document
    .getElementById("editArrivalSaveBtn")
    .addEventListener("click", commitArrivalEdit);
  document
    .getElementById("editArrivalResetBtn")
    .addEventListener("click", resetArrivalOverride);
  document
    .getElementById("editArrivalCancelBtn")
    .addEventListener("click", () => {
      document.getElementById("editArrivalModal").style.display = "none";
      pendingArrivalEdit = null;
    });
  document
    .getElementById("cancelDayScheduleBtn")
    .addEventListener("click", () => {
      const modal = document.getElementById("dayDetailModal");
      const dateStr = modal.getAttribute("data-current-date");
  ...
  document
    .getElementById("cancelWeekScheduleBtn")
    .addEventListener("click", () => {
      const modal = document.getElementById("dayDetailModal");
      const dateStr = modal.getAttribute("data-current-date");
...
document.addEventListener("DOMContentLoaded", () => {
  const safeInit = (label, fn) => {
    try {
      ...
      console.error(`PULSE init step failed: ${label}`, e);
    }
  };
  ...
  safeInit("reset auto group button", () => {
    document
      .getElementById("resetAutoGroupBtn")
      .addEventListener("click", () => {
        if (
          !confirm(
            "This clears every manual group assignment (including anything drawn on the map) and re-clusters everyone automatically. Continue?",
          )
        )
          return;
        patients.forEach((p) => {
  ...
  safeInit("CSV download/template buttons", () => {
    document
      ...
      .addEventListener("click", downloadAllPatientsCsv);
    document
      .getElementById("downloadTemplateBtn")
      .addEventListener("click", () => {
        const blob = new Blob(
          ["Name,Address,DOB,Coordinator,Provider,Last Visit\n"],
          { type: "text/csv" },
        );
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
  ...
  safeInit("clear all button", () => {
    document
      ...
      .addEventListener("click", showClearAllStep1);
  });

  safeInit("manual add row button", () => {
    document
      .getElementById("addManualRowBtn")
  ...
  safeInit("clients search box", () => {
    ...
    const updateSearchClearX = () => {
      searchClearX.style.display = clientsSearchEl.value ? "block" : "none";
    };
    clientsSearchEl.addEventListener("input", () => {
      renderTable();
      updateSearchClearX();
    });
    ...
    document
      .getElementById("clientsFilterCount")
  ...
  safeInit("schedule search box", () => {
    document
      ...
      .addEventListener("input", (e) =>
        renderScheduleSearchResults(e.target.value.trim()),
      );
  });

  safeInit("manual add form", () => {
    document
      .getElementById("submitManualAddBtn")
      .addEventListener("click", submitManualAdd);
